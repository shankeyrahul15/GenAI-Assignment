...
class ChatUI {
    ...
    async sendMessage() {
        ...
        try {
            ...
            // Read requested number of records for test data (if input exists)
            const testDataCountEl = document.getElementById('testDataCount');
            const numRecords = testDataCountEl ? Math.max(1, parseInt(testDataCountEl.value, 10) || 1) : 1;
    ...
    getPromptKeys(language, engine) {
        ...
        if (isFeatureChecked && isPageChecked) {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                // For Playwright + TypeScript: generate both feature file and a Playwright TS page
                promptKeys.push('CUCUMBER_ONLY');
                promptKeys.push('PLAYWRIGHT_TYPESCRIPT_PAGE_ONLY');
            } else {
                // For other non-supported combinations, generate feature only and warn
                promptKeys.push('CUCUMBER_ONLY');
        ...
        // If Test Data generation selected, always include the test data prompt last
